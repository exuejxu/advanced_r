...
for(k in 1:length(lat)){
  dis[k] = distHaversine(c(lat[k], lon[k]), c(a,b))/1000000 #compute and scale the distance 
  dist_kl = numeric(length(dis))
  #print(dis)
  for (j in 1:length(dis)){
    dist_kl[j] = exp(-(dis[j]/h_distance)^2)  
  }
}
dist_kl      
...
h_date = 5
dis = abs(date - datetest)/10000  #compute and scale the distance 
...
gkernel = function(X, Y, Xtest, h_time){
  ...
  for (i in 1:n){
    ...
    kl = time_kl + dist_kl[i] + date_kl[i]  #sum of three Gaussian kernels
    #print(kl)
