...
nc1 = length(CL1)  #length(CL1)=length(RW2)
nc2 = length(CL2)
...
pihat = nc1/nrow(data)  #0.5

w0i1 = -0.5*t(mu1)%*%solve(sigma)%*%mu1+log(pihat)
wi1 = solve(sigma)%*%mu1
w0i1
wi1

w0i2 = -0.5*t(mu2)%*%solve(sigma)%*%mu2+log(1-pihat)
wi2 = solve(sigma)%*%mu2
w0i2
wi2
...
which.min(valiScore[2:15]) 
# [1] 3 the 3rd element, not value 
# as it is from 2 so, 3+1=4

# report it¡¯s depth and the variables used by the tree.
...
prob1 = predict(fitNB, newdata = train) #get prob value
prob1

classLoss <- function(fit, train){
  p <- predict(fit, newdata=train, type="raw") #get prob value
  #              bad         good
